...
export default function FriendsRequestsScreen(){
    ...
    if (user.data?.status === "friends"){
        ...
        deleteFunction = (data) => {df(data); router.push("friends")}
        approveFunction = (data) => {router.push("chats")}
    }
    else if (user.data?.status === "request"){
        // statButton = "Підтвердити"
        deleteFunction = (data) => {dr(data); router.push("friends")}
        approveFunction = (data) => {mf(data); router.push("friends")}
    }
    else {
        deleteFunction = (data) => {router.push("friends")}
        approveFunction = (data) => {mr(data); router.push("friends")}
    }
